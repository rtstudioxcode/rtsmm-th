...
export function attachUser(req, res, next) {
  ...
  if (u) {
    const minimal = { _id: String(u._id || u.id), username: u.username, role: u.role || 'user' };
    req.user = minimal;
    // ✅ ถ้ามี res.locals.me อยู่แล้ว ให้ merge แทน ไม่เขียนทับ
    res.locals.me = res.locals.me ? { ...res.locals.me, ...minimal } : { ...minimal };
  }
  next();
}
...
function respondUnauthorized(req, res) {
  if (wantsJSON(req)) return res.status(401).json({ ok: false, message: 'Unauthenticated' });
...
function respondForbidden(req, res) {
  if (wantsJSON(req)) return res.status(403).json({ ok: false, message: 'Forbidden' });
...
export function requireVerified(req, res, next) {
  ...
  if (wantsJSON(req)) return res.status(403).json({ ok:false, message:'Email not verified' });
